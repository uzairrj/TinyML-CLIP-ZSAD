...
def metrics_eval(
    ...
    precision, recall, _ = precision_recall_curve(pixel_label, pixel_preds)
    f1_scores = 2 * (precision * recall) / (precision + recall + 1e-10)
    zero_pixel_max_f1 = np.max(f1_scores)
    # ================================================================================================
    # image level auc & ap & max-f1
    if image_label.max() != image_label.min():
        ...
        img_precision, img_recall, _ = precision_recall_curve(image_label, agg_image_preds)
        img_f1_scores = 2 * (img_precision * img_recall) / (img_precision + img_recall + 1e-10)
        agg_image_max_f1 = np.max(img_f1_scores)
    else:
        agg_image_auc = 0
        agg_image_ap = 0
        agg_image_max_f1 = 0
    ...
    result = {
        ...
        "pixel AUROC": round(zero_pixel_auc, 4) * 100,
        "pixel AP": round(zero_pixel_ap, 4) * 100,
        "pixel max-F1": round(zero_pixel_max_f1, 4) * 100,
        "image AUROC": round(agg_image_auc, 4) * 100,
        "image AP": round(agg_image_ap, 4) * 100,
        "image max-F1": round(agg_image_max_f1, 4) * 100,
    }
    return result
