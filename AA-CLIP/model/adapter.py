...
class AdaptedCLIP(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.levels = levels
        total_layers = len(self.image_encoder.transformer.resblocks)
        if max(self.levels) > total_layers:
            scale = total_layers / max(self.levels)
            self.levels = [int(l * scale) for l in self.levels]

        vision_width = clip_model.visual.conv1.out_channels
        text_width = clip_model.token_embedding.weight.shape[1]
        ...
        layer_adapters = nn.ModuleList(
            [SimpleAdapter(vision_width, vision_width) for _ in range(image_adapt_until)]
        )
        seg_proj = nn.ModuleList(
            [SimpleProj(vision_width, embed_dim, relu) for _ in range(len(levels))]
        )
        det_proj = SimpleProj(vision_width, embed_dim, relu)
        ...
        self.text_adapter = nn.ModuleList(
            [SimpleAdapter(text_width, text_width) for _ in range(text_adapt_until)]
            + [SimpleProj(text_width, embed_dim, relu=True)]
        )
        self._init_weights_()
    ...
    def forward_original(self, x, modality="visual"):
        if modality == "visual":
            cls_features, patch_features = self.clipmodel.encode_image(x, [len(self.image_encoder.transformer.resblocks)])
            patch_features = [
    ...
    def forward(self, x):
        ...
        tokens = []
        for i in range(len(self.image_encoder.transformer.resblocks)):
