...
class MyDataset(data.Dataset):
	...
	def combine_img(self, cls_name):   # From APRIL-GAN: https://github.com/ByChelsea/VAND-APRIL-GAN
		...
		for i in range(4):
			...
			defect = [d for d in defect if d != "good"]
			files = [
					f'{d}/{f}'
					for d in defect
					for f in os.listdir(os.path.join(img_paths, d))
				]
			random_file = random.choice(files)
			img_path = os.path.join(img_paths, random_file)
			mask_path = os.path.join(self.root, cls_name, 'ground_truth', random_file.replace(".png", "_mask.png"))
