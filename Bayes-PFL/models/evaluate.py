...
def evaluate_pre(val_dataloader, model_clip, MyModel, device, args, val_obj_list_post, PFL_TextEncoder, tokenizer, stage):
    ...
    for obj in val_obj_list_post:
        ...
        try:
            auroc_sp = roc_auc_score(gt_sp.ravel(), pr_sp.ravel())
        except ValueError:
            auroc_sp = 0.5
        auroc_sp_ls.append(auroc_sp)

        precisions_px, recalls_px, _ = precision_recall_curve(gt_px.ravel(), pr_px.ravel())
        f1_scores_px = (2 * precisions_px * recalls_px) / (precisions_px + recalls_px + 1e-8)
        f1_px = np.max(f1_scores_px[np.isfinite(f1_scores_px)])
        f1_px_ls.append(f1_px)

        try:
            precisions_sp, recalls_sp, _ = precision_recall_curve(gt_sp.ravel(), pr_sp.ravel())
            f1_scores_sp = (2 * precisions_sp * recalls_sp) / (precisions_sp + recalls_sp + 1e-8)
            f1_sp = np.max(f1_scores_sp[np.isfinite(f1_scores_sp)])
        except ValueError:
            f1_sp = 0.0
    ...
    auroc_mean_pixel = np.mean(auroc_px_ls)
    auroc_mean_image = np.mean(auroc_sp_ls)
    f1_mean_pixel = np.mean(f1_px_ls)
    f1_mean_image = np.mean(f1_sp_ls)
    ...
    return ap_mean_pixel, ap_mean_image, auroc_mean_pixel, auroc_mean_image, f1_mean_pixel, f1_mean_image