...
def train(args):
    ...
    # -------------------------------------------------------------------------------------------------
    if '39M' in args.model:
        pretrained_tag = 'yfcc15m'
    else:
        pretrained_tag = 'laion400m' if 'TinyCLIP' in args.model else 'openai'
    model_clip, _, _ = open_clip.create_model_and_transforms(args.model, pretrained=pretrained_tag, img_size=image_size) 
    tokenizer = open_clip.get_tokenizer(args.model)
    ...
    for epoch_post in range(epochs):
        ...
        if (epoch_post + 1) > 0:
            MyModel.zk_n = None
            MyModel.zk_a = None
            ap_raw_pixel, ap_raw_image, auroc_raw_pixel, auroc_raw_image, f1_raw_pixel, f1_raw_image = evaluate_pre(val_post_dataloader, model_clip, MyModel, device, args, val_obj_list_post, PFL_TextEncoder, tokenizer, stage)

        if (epoch_post + 1) % args.print_freq == 0:
            logger.info('epoch post [{}], loss_seg:{:.4f} loss_cls:{:.4f} loss_dist_reg:{:.4f}  loss_text:{:.4f} ap_raw_pixel:{:.4f}  ap_raw_image:{:.4f} auroc_pixel:{:.4f} auroc_image:{:.4f} f1_pixel:{:.4f} f1_image:{:.4f}'.format(epoch_post + 1 , np.mean(loss_seg_list), np.mean(loss_class_list),np.mean(loss_dist_reg_list),np.mean(loss_text_list), ap_raw_pixel, ap_raw_image, auroc_raw_pixel, auroc_raw_image, f1_raw_pixel, f1_raw_image))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", type=str, default="TinyCLIP-ViT-39M-16-Text-19M", help="model used")
    ...
    parser.add_argument("--config_path", type=str, default='./open_clip_local/model_configs/TinyCLIP-ViT-39M-16-Text-19M.json', help="model configs")
