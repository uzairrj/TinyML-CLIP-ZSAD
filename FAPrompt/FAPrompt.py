...
class FAPrompt(nn.Module):
    def __init__(self, clip_model, design_details):
        ...
        self.embed_dim = clip_model.visual.proj.shape[1] if clip_model.visual.proj is not None else ctx_dim
        self.classnames = classnames

        self.patch_meta_net = nn.Sequential(OrderedDict([
            ("linear1", nn.Linear(self.embed_dim * self.k, self.embed_dim * self.k // 16)),
            ("relu", nn.ReLU(inplace=True)),
            ("linear2", nn.Linear(self.embed_dim * self.k // 16, ctx_dim))
        ]))
    ...
    def forward(self, selected_tokens = None):
        ...
        if selected_tokens != None:
            selected_tokens = selected_tokens.reshape(1, self.embed_dim * self.k)
