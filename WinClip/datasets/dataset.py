...
class CLIPDataset(data.Dataset):
    def __init__(self, root, transform, target_transform, dataset_name, mode='test', class_name=None):
        self.root = root
        ...
        for cls_name in self.cls_names:
            if class_name is not None and class_name != cls_name:
                continue
    ...
    def __getitem__(self, index):
        ...
        gt = np.array(img_mask, dtype=np.uint8)

        img_cv = cv2.resize(img_cv, (1024, 1024))
        gt = cv2.resize(gt, (1024, 1024), interpolation=cv2.INTER_NEAREST)

        img_name = f'{cls_name}-{specie_name}-{os.path.basename(img_path[:-4])}'

        return img_cv, gt, anomaly, img_name, specie_name
